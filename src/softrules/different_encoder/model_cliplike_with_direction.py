...
class SoftRulesEncoder(pl.LightningModule):
    ...
    def on_validation_epoch_end(self, *args, **kwargs):
        ...
        for key in self.val_step_outputs.keys():
            ...
            if self.hyperparameters.get("save_predictions", None):
                with open(self.hyperparameters["save_predictions"], 'a+') as fout:
                    print(pred_scores)
                    print(relations)
                    print(gold)
                    print(self.hyperparameters)
                    dump = {'pred_scores': pred_scores.to_list(), 'relations': relations, 'gold': gold, 'args': self.hyperparameters}
                    _=fout.write(json.dumps(dump))
                    _=fout.write('\n')
    ...
    for episode, selections, relations in zip(val_data[0], val_data[1], val_data[2]):
        for ts in episode['meta_test']:
            ...
            for rule, ss_relation in zip(episode_ss_rules, relations):
                ...
                # TODO small issue here; add to `entity_to_id`, not `entity_types_to_id`
