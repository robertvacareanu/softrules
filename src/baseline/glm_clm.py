...
def sents_to_glm_data_input(sent_a, sent_b, rel_a: Optional[str]=None, rel_b: Optional[str]=None):
    if rel_a and rel_b:
        answer = 'Yes' if rel_a == rel_b else 'No'
...
def construct_training_dataset(kwargs):
    ...
    for i in range(kwargs['finetuning_examples'] // 4):
        ...
        data.append({
            'input' : sents_to_glm_data_input(sent_a_tokens, sent_b_tokens, rel_a = sent_a['relation'], rel_b = sent_b['relation']),
        })
        ...
        for _ in range(3):
            ...
            data.append({
                'input' : sents_to_glm_data_input(sent_a_tokens, sent_c_tokens, rel_a = sent_a['relation'], rel_b = sent_b['relation']),
            })
...
def main(args):
    ...
    train_data = construct_training_dataset(args).map(lambda x: {**tokenizer(x['input'], truncation=True, max_length=384), }, batched=True, load_from_cache_file=False, remove_columns=['input'])
    ...
    for evaluation_path in args['evaluation_paths']:
        ...
        eval_dataset = construct_eval_dataset_from_episodes({**args, 'all_episodes': test_episodes})

        eval_dataset_hf = datasets.Dataset.from_list(eval_dataset)
        preds = trainer.predict(eval_dataset_hf.map(lambda x: {**tokenizer(x['input'], truncation=True, max_length=384), }, batched=True, load_from_cache_file=False, remove_columns=eval_dataset_hf.column_names), max_new_tokens=10, pad_token_id=tokenizer.eos_token_id)
        preds.predictions[preds.predictions == -100] = 0
...
def get_huggingface_args(parent_parser):
    ...
    subparser.add_argument('--output_dir',                  type=str,   default='logs/clm/',  help='Where to save the model')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_name',             type=str, default='gpt2', help="What model to use")
